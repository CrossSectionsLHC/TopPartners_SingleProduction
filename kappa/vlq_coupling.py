...
gamma_eq = sp.Eq(M_X**2/246**2*    3 * (kappa**2*X_L + kappa**2*X_R) * lambda_ / (96 * np.pi * M_X**3) * (m_q**2 + M_X**2 - M_boson**2 + 4 * kappa**2*X_L*kappa**2*X_R*m_q*M_X/(kappa**2*X_L + kappa**2*X_R)), Gamma) #T->Ht
...
for M_X_arr, Gamma_arr, kappa_arr in zip(M_X_arr, Gamma_arr, kappa_arr):
    print(f"M_X = {M_X_arr},	Gamma = {Gamma_arr},	kappa = {kappa_arr}")
